...
export default class ModalDropdown extends Component {
	...
	static getDerivedStateFromProps (nextProps, state) {
		...
		buttonText = state.nextValue == null ? buttonText : state.nextValue;
		selectedIndex = state.nextIndex == null ? selectedIndex : state.nextIndex;
		if (selectedIndex < 0) {
	...
	_renderRow = ({item: rowData, index: rowID, separators}) => {
		...
		const rtype = 'TouchableHighlight';
		if (TOUCHABLE_ELEMENTS.find(name => name == rtype)){ //row.type.displayName)) {
			const props = {...row.props};
			props.key = preservedProps.key;
			props.onPress = preservedProps.onPress;
			const {children} = row.props;
			switch (rtype) { // row.type.displayName) {
	...
	_onRowPress(rowData, rowID) {
		//console.log("_onRowPress", rowData, rowID);
